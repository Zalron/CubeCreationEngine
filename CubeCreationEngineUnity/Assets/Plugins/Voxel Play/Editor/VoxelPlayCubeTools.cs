...
namespace VoxelPlay {
	...
	public class VoxelPlayCubeTools : EditorWindow {
		...
		public enum CUBE_SHADING_STYLE {
			Color = 0,
			Textured = 1,
			TexturedAlpha = 2
		}

		public enum CUBE_VERTEX_OFFSET_OPTION {
			Custom = 0,
		...
		[SerializeField]
		public CUBE_SHADING_STYLE cubeShadingStyle = CUBE_SHADING_STYLE.Color;
		...
		[SerializeField]
		public CUBE_VERTEX_OFFSET_OPTION offsetOption = CUBE_VERTEX_OFFSET_OPTION.Custom;
		...
		void OnGUI () {
			...
			cubeShadingStyle = (CUBE_SHADING_STYLE)EditorGUILayout.EnumPopup (cubeShadingStyle);
			EditorGUILayout.EndHorizontal ();
			...
			offsetOption = (CUBE_VERTEX_OFFSET_OPTION)EditorGUILayout.EnumPopup (offsetOption);
			EditorGUILayout.EndHorizontal ();

			switch (offsetOption) {
			case CUBE_VERTEX_OFFSET_OPTION.DoorOpensToTheLeft:
				offset = new Vector3 (0.5f - scale.z * 0.5f, scale.y * 0.5f, 0);
				break;
			case CUBE_VERTEX_OFFSET_OPTION.DoorOpensToTheRight:
				offset = new Vector3 (-0.5f + scale.z * 0.5f, scale.y * 0.5f, 0);
				break;
			}

			GUI.enabled = (offsetOption == CUBE_VERTEX_OFFSET_OPTION.Custom);
			EditorGUILayout.BeginHorizontal ();
			...
			if (offsetOption != CUBE_VERTEX_OFFSET_OPTION.Custom) {
				EditorGUILayout.BeginHorizontal ();
			...
			if (cubeShadingStyle != CUBE_SHADING_STYLE.Color) {
				EditorGUILayout.BeginHorizontal ();
		...
		GameObject GenerateCubePrefab () {
			...
			Material mat;
			if (cubeShadingStyle == CUBE_SHADING_STYLE.Textured) {
				mat = Resources.Load<Material> ("VoxelPlay/Materials/VP Model Texture");
			} else if (cubeShadingStyle == CUBE_SHADING_STYLE.TexturedAlpha) {
            ...
            if (cubeShadingStyle != CUBE_SHADING_STYLE.Color && mat.mainTexture != null) {
		...
		VoxelDefinition GenerateDoorVoxel () {
			...
			if (offsetOption == CUBE_VERTEX_OFFSET_OPTION.DoorOpensToTheLeft) {
				door.customTag = "left";
			}
			...
			if (offsetOption == CUBE_VERTEX_OFFSET_OPTION.DoorOpensToTheRight) {
