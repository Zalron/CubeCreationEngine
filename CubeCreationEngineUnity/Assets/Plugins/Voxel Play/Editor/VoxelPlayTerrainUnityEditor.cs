...
namespace VoxelPlay {
	...
	public class VoxelPlayTerrainUnityEditor : Editor {
		...
		public override void OnInspectorGUI () {
			...
			if (tg.terrainData != null) {
				...
				for (int k = 0; k < tg.terrainData.splatPrototypes.Length; k++) {
					...
					tg.splatSettings [k].action = (UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION)EditorGUILayout.EnumPopup (tg.splatSettings [k].action);
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.EndVertical ();
					EditorGUILayout.EndHorizontal ();
					if (tg.splatSettings [k].action != UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Ignore)
						allIgnored = false;
					if (tg.splatSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
						needCreate = true;
					} else if ((tg.splatSettings [k].top == null || tg.splatSettings [k].dirt == null) && tg.splatSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned) {
						needAssign = true;
					}
					EditorGUILayout.Separator ();
				}
				...
				for (int k = 0; k < tg.terrainData.treePrototypes.Length; k++) {
					...
					EditorGUILayout.BeginHorizontal ();
					EditorGUILayout.LabelField ("Action", GUILayout.Width (80));
					tg.treeSettings [k].action = (UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION)EditorGUILayout.EnumPopup (tg.treeSettings [k].action);
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.EndVertical ();
					EditorGUILayout.EndHorizontal ();
					if (tg.treeSettings [k].action != UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Ignore)
						allIgnored = false;
					if (tg.treeSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
						needCreate = true;
					} else if (tg.treeSettings [k].md == null && tg.treeSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned) {
						needAssign = true;
					}
					EditorGUILayout.Separator ();
				}
				...
				for (int k = 0; k < tg.terrainData.detailPrototypes.Length; k++) {
					...
					EditorGUILayout.BeginHorizontal ();
					EditorGUILayout.LabelField ("Action", GUILayout.Width (80));
					tg.detailSettings [k].action = (UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION)EditorGUILayout.EnumPopup (tg.detailSettings [k].action);
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.EndVertical ();
					EditorGUILayout.EndHorizontal ();
					if (tg.detailSettings [k].action != UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Ignore)
						allIgnored = false;
					if (tg.detailSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
						needCreate = true;
					} else if (tg.detailSettings [k].vd == null && tg.detailSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned) {
						needAssign = true;
					}
					EditorGUILayout.Separator ();
				}
		...
		void GenerateTerrainVoxels (bool forceGeneration) {
            ...
            for (int k = 0; k < tg.terrainData.splatPrototypes.Length; k++) {
#endif
                if (forceGeneration || tg.splatSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
					if (tg.splatSettings [k].preview == null)
						continue;
					...
					tg.splatSettings [k].action = UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned;
				}
			}
		}
		...
		void GenerateVegetationVoxels (bool forceGeneration) {
			...
			CheckDirectory (path);
			for (int k = 0; k < tg.terrainData.detailPrototypes.Length; k++) {
				if (forceGeneration || tg.detailSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
					...
					tg.detailSettings [k].action = UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned;
				}
			}
		}
		...
		void GenerateTreeModels (bool forceGeneration) {
			...
			for (int k = 0; k < tg.terrainData.treePrototypes.Length; k++) {
				if (forceGeneration || tg.treeSettings [k].action == UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Create) {
					// Get tree size
					...
					tg.treeSettings [k].action = UnityTerrainGenerator.TERRAIN_RESOURCE_ACTION.Assigned;
				}
			}
		}
