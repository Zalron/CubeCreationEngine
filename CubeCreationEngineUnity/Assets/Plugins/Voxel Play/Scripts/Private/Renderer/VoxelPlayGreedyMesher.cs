...
namespace VoxelPlay
{
	...
	public class VoxelPlayGreedyMesher
	{
		...
		public void AddQuad (FACE_DIRECTION direction, int x, int y, int slice)
		{
			int index = (int)direction * 16 + slice;
			slices [index].AddQuad (x, y);
		}

		public void AddQuad (FACE_DIRECTION direction, int x, int y, int slice, ref Vector3 normal, ref Color32 color, float light, int textureIndex)
		{
			int index = (int)direction * 16 + slice;
		...
		public void FlushTriangles (List<Vector3> vertices, List<int>indices)
		{
			for (int d=0;d<6;d++) {
				for (int s = 0; s < 16; s++) {
					slices [d * 16 + s].FlushTriangles ((FACE_DIRECTION)d, s, vertices, indices);
				}
			}
		}
		...
		public void FlushTriangles (List<Vector3> vertices, List<int>indices, List<Vector4>uv0, List<Vector3>normals, List<Color32>colors)
		{
			for (int d=0;d<6;d++) {
				for (int s = 0; s < 16; s++) {
					slicesFull [d * 16 + s].FlushTriangles ((FACE_DIRECTION)d, s, vertices, indices, uv0, normals, colors);
				}
			}
		}
