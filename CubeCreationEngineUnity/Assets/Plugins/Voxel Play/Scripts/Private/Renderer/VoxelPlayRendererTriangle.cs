...
//		};
		...
		void GenerateMeshData_Triangle (int jobIndex)
		{
			...
			waterIndices = meshJobs [jobIndex].waterIndices;
						...
						case RenderType.Water:
							{
								...
								if (occ != 1) {
									AddFaceWater (faceVerticesBottom, normalsDown, pos, waterIndices, type.textureIndexSide, light + noflow, 0, 0, 0, 0, tintColor);
								}
							}
							break;
						...
						case RenderType.Transp6tex:
							{
								...
								if (v1b != FULL_OPAQUE && chunk_middle_back_middle [middle_back_middle].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesBack, normalsBack, pos, transpIndices, type.textureSideIndices [rotationIndex].back, light, tintColor);
									if (enableColliders) {
								...
								if (v1f != FULL_OPAQUE && chunk_middle_forward_middle [middle_forward_middle].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesForward, normalsForward, pos, transpIndices, type.textureSideIndices [rotationIndex].forward, light, tintColor);
									if (enableColliders) {
								...
								if (v1u != FULL_OPAQUE && chunk_top_middle_middle [top_middle_middle].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesTop, normalsUp, pos, transpIndices, type.textureIndexTop, light, tintColor);
									if (enableColliders) {
								...
								if (v1d != FULL_OPAQUE && chunk_bottom_middle_middle [bottom_middle_middle].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesBottom, normalsDown, pos, transpIndices, type.textureIndexBottom, light, tintColor);
									if (enableColliders) {
										greedyCollider.AddQuad (FACE_DIRECTION.Bottom, x, z, y);
										if (enableNavMesh && type.navigatable) {
											greedyNavMesh.AddQuad (FACE_DIRECTION.Bottom, x, z, y);
										}
									}
								}
								...
								if (v1l != FULL_OPAQUE && chunk_middle_middle_left [middle_middle_left].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesLeft, normalsLeft, pos, transpIndices, type.textureSideIndices [rotationIndex].left, light, tintColor);
									if (enableColliders) {
								...
								if (v1r != FULL_OPAQUE && chunk_middle_middle_right [middle_middle_right].typeIndex != typeIndex) {
									AddFaceTransparent (faceVerticesRight, normalsRight, pos, transpIndices, type.textureSideIndices [rotationIndex].right, light, tintColor);
									if (enableColliders) {
						...
						case RenderType.Cutout: // Opaque & Cutout
							...
							if (enableSmoothLighting) {
								...
								if (v1b < FULL_OPAQUE || denseTreeCheck) {												
									...
									if (isOpaqueType) {
										greedyNoAO.AddQuad (FACE_DIRECTION.Back, x, y, z, ref Misc.vector3back, ref tintColor, backFaceGI, type.textureSideIndices [rotationIndex].back + windAnimation);
									} else {
										greedyCutout.AddQuad (FACE_DIRECTION.Back, x, y, z, ref Misc.vector3back, ref tintColor, backFaceGI, type.textureSideIndices [rotationIndex].back + windAnimation);
									}
									if (addCollider) {
										greedyCollider.AddQuad (FACE_DIRECTION.Back, x, y, z);
										if (enableNavMesh && type.navigatable) {
								...
								if (v1f < FULL_OPAQUE || denseTreeCheck) {
									...
									if (isOpaqueType) {
										greedyNoAO.AddQuad (FACE_DIRECTION.Forward, x, y, z, ref Misc.vector3forward, ref tintColor, frontFaceGI, type.textureSideIndices [rotationIndex].forward + windAnimation);
									} else {
										greedyCutout.AddQuad (FACE_DIRECTION.Forward, x, y, z, ref Misc.vector3forward, ref tintColor, frontFaceGI, type.textureSideIndices [rotationIndex].forward + windAnimation);
									}
									if (addCollider) {
										greedyCollider.AddQuad (FACE_DIRECTION.Forward, x, y, z);
										if (enableNavMesh && type.navigatable) {
								...
								if (v1l < FULL_OPAQUE || denseTreeCheck) {
									...
									if (isOpaqueType) {
										greedyNoAO.AddQuad (FACE_DIRECTION.Left, z, y, x, ref Misc.vector3left, ref tintColor, leftFaceGI, type.textureSideIndices [rotationIndex].left + windAnimation);
									} else {
										greedyCutout.AddQuad (FACE_DIRECTION.Left, z, y, x, ref Misc.vector3left, ref tintColor, leftFaceGI, type.textureSideIndices [rotationIndex].left);
									}
									if (addCollider) {
										greedyCollider.AddQuad (FACE_DIRECTION.Left, z, y, x);
										if (enableNavMesh && type.navigatable) {
								...
								if (v1r < FULL_OPAQUE || denseTreeCheck) {
									...
									if (isOpaqueType) {
										greedyNoAO.AddQuad (FACE_DIRECTION.Right, z, y, x, ref Misc.vector3right, ref tintColor, rightFaceGI, type.textureSideIndices [rotationIndex].right + windAnimation);
									} else {
										greedyCutout.AddQuad (FACE_DIRECTION.Right, z, y, x, ref Misc.vector3right, ref tintColor, rightFaceGI, type.textureSideIndices [rotationIndex].right + windAnimation);
									}
									if (addCollider) {
										greedyCollider.AddQuad (FACE_DIRECTION.Right, z, y, x);
										if (enableNavMesh && type.navigatable) {
			...
			meshJobs [jobIndex].waterVoxelsCount = waterVoxelsCount;
			if (waterVoxelsCount > 0) {
				subMeshCount++;
				matVoxelIndex += 4;
			}
