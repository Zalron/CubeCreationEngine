...
inline void PushCorner(inout g2f i, inout TriangleStream<g2f>o, float3 center, float3 corner, float4 uv) {
	...
	VOXELPLAY_SET_VERTEX_LIGHT(i, wpos, norm)

    #if defined(USE_WORLD_SPACE_UV)
    uv.xy = wpos.xz * norm.y + wpos.xy * norm.z + wpos.zy * norm.x;
...
void PushVoxel(float3 center, int4 iuv, int4 occi, inout TriangleStream<g2f> o) {
	...
	float3 v3          = cubeVerts[3];
		...
		float disp = (iuv.w >> 16) * sin(worldCenter.x + worldCenter.y + _Time.w) * _VPTreeWindSpeed;
		v0.xy += disp;
		v1.xy += disp;
		v2.xy += disp;
		v3.xy += disp;
