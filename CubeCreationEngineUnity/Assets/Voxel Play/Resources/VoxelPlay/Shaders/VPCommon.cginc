...
fixed _VPGrassWindSpeed, _VPTreeWindSpeed;
	...
	#define VOXELPLAY_APPLY_TINTCOLOR(color, i)
        ...
        #define VOXELPLAY_GET_TEXEL_GEO(uv) ReadSmoothTexel2D(uv.xy)
    ...
    #if defined(NON_ARRAY_TEXTURE)
        #define VOXELPLAY_GET_TEXEL_GEO(uv) tex2D(_MainTex, uv.xy)
...
inline float3 GetLight() {
	...
	inline float3 GetLight(float3 normal) {
        #if defined(SUN_SCATTERING)
            const float3 NdotL3 = 0.7.xxx; //saturate(_WorldSpaceLightPos0.xyz * normal * 0.5 + 0.5);
        #else
...
// note: _VPAmbientLight could be left outside of saturate() function. In that case AO will be affected (diminished due to atten * ao calc, see VOXELPLAY_APPLY_LIGHTING_AO_AND_GI function below) so we leave it inside.
    ...
    #define VOXELPLAY_LIGHT_ATTENUATION(i) max(0, (1.0 + _WorldSpaceLightPos0.y * _VPDaylightShadowAtten) * i.light + _VPAmbientLight)
...
#define VOXELPLAY_APPLY_LIGHTING(color,i) fixed atten = VOXELPLAY_LIGHT_ATTENUATION(i); color.rgb *= atten * _LightColor0.rgb + VOXELPLAY_VERTEX_LIGHT_COLOR(atten * UNITY_SHADOW_ATTEN(i) );
#define VOXELPLAY_APPLY_LIGHTING_AO_AND_GI(color,i) fixed atten = VOXELPLAY_LIGHT_ATTENUATION(i); float ao = i.uv.w; ao = 1.0-(1.0-ao)*(1.0-ao); color.rgb *= (atten * ao) * _LightColor0 + VOXELPLAY_VERTEX_LIGHT_COLOR(i.uv.w * UNITY_SHADOW_ATTEN(i) );
#define VOXELPLAY_APPLY_LIGHTING_AND_GI(color,i) fixed atten = VOXELPLAY_LIGHT_ATTENUATION(i); color.rgb *= (atten * i.uv.w) * _LightColor0.rgb + VOXELPLAY_VERTEX_LIGHT_COLOR(i.uv.w * UNITY_SHADOW_ATTEN(i) );
