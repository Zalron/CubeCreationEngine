...
namespace VoxelPlay
{
    ...
    public abstract class MeshingThread
    {
        ...
        public FastFixedBuffer<int> [] tempGeoIndices;
        ...
        public virtual void Init (int threadId, int poolSize, VoxelPlayEnvironment env)
        {
            ...
            int initialCapacity = env.geometryShadersActive ? 2048 : 15000;
            for (int k = 0; k < meshJobs.Length; k++) {
                ...
                if (env.geometryShadersActive) {
                    meshJobs [k].uv2 = Misc.GetList<Vector4> (lowMemoryMode, initialCapacity);
                } else {
            ...
            if (env.geometryShadersActive) {
                tempGeoIndices = new FastFixedBuffer<int> [VoxelPlayEnvironment.MAX_MATERIALS_PER_CHUNK];
                for (int k = 0; k < tempGeoIndices.Length; k++) {
