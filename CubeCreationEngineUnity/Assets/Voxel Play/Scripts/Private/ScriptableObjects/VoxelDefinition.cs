...
namespace VoxelPlay
{
    ...
    public static class RenderTypeExtensions
    {
        ...
        public static Material GetDefaultMaterial (this RenderType o, VoxelPlayEnvironment context, bool geometryShaderAllowed = true)
        {
            string name;
            bool compatibleWithGeometryShaders = geometryShaderAllowed && context.geometryShadersActive;
            ...
            case RenderType.Water:
                if (context.realisticWater) {
                    if (shadowsOnWater) {
                        name = compatibleWithGeometryShaders ? "VP Voxel Geo Water Realistic" : "VP Voxel Triangle Water Realistic";
                    } else {
                        name = compatibleWithGeometryShaders ? "VP Voxel Geo Water Realistic No Shadows" : "VP Voxel Triangle Water Realistic No Shadows";
                    }
                } else {
                    if (shadowsOnWater) {
                        name = compatibleWithGeometryShaders ? "VP Voxel Geo Water" : "VP Voxel Triangle Water";
                    } else {
            ...
            case RenderType.Transp6tex:
                if (context.doubleSidedGlass) {
                    name = compatibleWithGeometryShaders ? "VP Voxel Geo Transp Double Sided" : "VP Voxel Triangle Transp Double Sided";
                } else {
                    name = compatibleWithGeometryShaders ? "VP Voxel Geo Transp" : "VP Voxel Triangle Transp";
                }
                break;
            case RenderType.OpaqueNoAO:
                name = compatibleWithGeometryShaders ? "VP Voxel Geo Opaque No AO" : "VP Voxel Triangle Opaque No AO";
                break;
            default:
