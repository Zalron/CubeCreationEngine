    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        void InitHeightMap ()
        {
            if (heightMapCache == null) {
                int poolSize = (visibleChunksDistance + 10) * 2 * 16 / 128 + 1;
                poolSize *= poolSize;
        ...
        void CheckChunksInRange (long endTime)
        {
            ...
            FastMath.FloorToInt (currentAnchorPos.x / 16f, currentAnchorPos.y / 16f, currentAnchorPos.z / 16f, out chunkX, out chunkY, out chunkZ);
        ...
        void ChunkRequestRefresh (Bounds bounds, bool clearLightmap, bool refreshMesh)
        {
            ...
            int xmin, ymin, zmin;
            FastMath.FloorToInt (min.x / 16f, min.y / 16f, min.z / 16f, out xmin, out ymin, out zmin);
            xmin *= 16;
            ymin *= 16;
            zmin *= 16;
            Vector3 max = bounds.max;
            int xmax, ymax, zmax;
            FastMath.FloorToInt (max.x / 16f, max.y / 16f, max.z / 16f, out xmax, out ymax, out zmax);
            xmax *= 16;
            ymax *= 16;
            zmax *= 16;

            for (int y = ymax; y >= ymin; y -= 16) {
                position.y = y;
