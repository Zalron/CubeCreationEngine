...
namespace VoxelPlay
{
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        int DamageAreaFast (Vector3 origin, int damage, int damageRadius = 1, bool distanceAttenuation = true, bool addParticles = true, List<VoxelIndex> results = null, bool playSound = false)
        {
            ...
            for (int k = 0; k < count; k++) {
                ...
                if (dam > 0 && GetVoxelVisibility (otherChunk, otherIndex)) {
                    FastVector.NormalizedDirection (ref origin, ref vi.position, out direction);
                    if (RayCastFast (origin, direction, out hitInfo, damageRadius, false, 5, ColliderTypes.OnlyVoxels)) {
                        int damageTaken = DamageVoxelFast (ref hitInfo, dam, addParticles, playSound);
                        if (hasResults) {
                            VoxelIndex di = vi;
                ...
                for (int k = 0; k < particlesAmount; k++) {
                    ...
                    if (destroyed) {
                        ...
                        if (voxelType.renderType == RenderType.CutoutCross) {   // smaller particles for vegetation
                            ...
                            startScale = new Vector3 (rnd, rnd, rnd);
                            endScale = new Vector3 (rnd * 0.6f, rnd * 0.6f, rnd * 0.6f);
                        } else {
                            float rnd = Random.Range (0.04f, 0.1f);
                            startScale = new Vector3 (rnd * 4f, rnd * 4f, rnd * 4f);
                            endScale = new Vector3 (rnd, rnd, rnd);
                        }
                        particleRenderer.transform.localScale = startScale;
                        particlePool [ppeIndex].endScale = endScale;
                    } else {
                        float rnd = Random.Range (0.03f, 0.06f);
                        Vector3 size = new Vector3 (rnd, rnd, rnd);
                        particleRenderer.transform.localScale = size;
                        particlePool [ppeIndex].endScale = size;
                    }
                    ...
                    if (destroyed) {
                        ...
                        Vector3 pos = hitInfo.voxelCenter;
                        FastVector.Add (ref pos, ref expelDir, 0.6f);
