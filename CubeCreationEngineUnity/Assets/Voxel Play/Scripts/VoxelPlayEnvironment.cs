...
namespace VoxelPlay
{
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        public bool GetVoxelIndex (ref VoxelIndex index, int offsetX, int offsetY, int offsetZ, out VoxelIndex otherIndex, bool createChunkIfNotExists = false)
        {
            ...
            pos.y = pos.y - 8 + py + offsetY;
            pos.z = pos.z - 8 + pz + offsetZ;
            pos.x = pos.x - 8 + px + offsetX;
        ...
        public bool GetVoxelIndex (VoxelChunk chunk, int voxelIndex, int offsetX, int offsetY, int offsetZ, out VoxelChunk otherChunk, out int otherVoxelIndex, bool createChunkIfNotExists = false)
        {
            ...
            if (GetChunkFast (chunkX, chunkY, chunkZ, out otherChunk, createChunkIfNotExists)) {
                py = (int)(position.y - chunkY * 16);
                pz = (int)(position.z - chunkZ * 16);
                px = (int)(position.x - chunkX * 16);
        ...
        /// </summary>
        public void ChunkRedrawAll ()
        {
            float d = _visibleChunksDistance * 16;
            Vector3 size = new Vector3 (d, d, d);
            ChunkRequestRefresh (new Bounds (currentAnchorPos, size), true, true);
        }
        ...
        public bool VoxelHighlight (VoxelHitInfo hitInfo, Color color, float edgeWidth = 0f)
        {
            ...
            if (hitInfo.placeholder != null) {
                ...
                if (hitInfo.placeholder.modelMeshRenderer != null) {
                    ht.position = hitInfo.placeholder.modelMeshRenderer.bounds.center;
                } else {
            ...
            if (createIfNotExists) {
                ...
                if (chunk.voxels [voxelIndex].hasContent == 1) {
                    ...
                    if (!voxelDefinition.isDynamic) {
                        ...
                        if (rot != 0) {
                            placeholder.transform.localRotation = Quaternion.Euler (0, rot, 0) * placeholder.transform.localRotation;
                        }
        ...
        public GameObject TorchAttach (VoxelHitInfo hitInfo, ItemDefinition torchDefinition = null)
        {
            ...
            return TorchAttachInt (hitInfo, torchDefinition);
        }
        ...
        /// <returns></returns>
        public GameObject TorchAttach (Vector3 worldPos, Vector3 normal, ItemDefinition torchDefinition = null)
        {
            if ((object)torchDefinition != null && torchDefinition.category != ItemCategory.Torch) {
                //Not a torch item
                return null;
